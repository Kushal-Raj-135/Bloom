...
function initializeMap() {
    try {
        ...
        searchInput?.addEventListener('input', (e) => {
            ...
            searchTimeout = setTimeout(async () => {                try {
                    const response = await fetch(`/api/geocoding/search/${encodeURIComponent(query)}`);
...
// Weather API Configuration - Now using secure backend proxy
...
async function fetchWeatherData() {
    try {
        ...
        const response = await fetch(`${WEATHER_API_URL}/weather/${encodeURIComponent(location)}?days=5&aqi=yes&alerts=yes`);
        const data = await response.json();
        
        // Update the weather forecast display
        const forecastContainer = document.querySelector('#weather-data');
...
// Load weather data function (wrapper around fetchWeatherData)
function loadWeatherData() {
    // Call the existing fetchWeatherData function
    fetchWeatherData();
...
// Load bioengineering system (wrapper around initializeBioengineering)
function loadBioengineering() {
    // Initialize the bioengineering system
    initializeBioengineering();
    
    // Set up any additional loading or initialization
    const bioSection = document.getElementById('bioengineering-section');
    if (bioSection) {
        // Add loading state initially
        bioSection.classList.add('loading');
...
// Calculate sustainability metrics based on crop type, soil type, and pH level
function calculateSustainabilityMetrics(cropType, soilType, phLevel) {
    // Base scores for different crop types
    const cropScores = {
        'wheat': { soilHealth: 75, carbonImpact: 65, waterEfficiency: 70, biodiversity: 60 },
        'rice': { soilHealth: 60, carbonImpact: 50, waterEfficiency: 40, biodiversity: 55 },
        'corn': { soilHealth: 70, carbonImpact: 55, waterEfficiency: 60, biodiversity: 50 },
        'soybean': { soilHealth: 85, carbonImpact: 80, waterEfficiency: 75, biodiversity: 85 },
        'cotton': { soilHealth: 50, carbonImpact: 45, waterEfficiency: 35, biodiversity: 40 }
    };

    // Soil type modifiers
    const soilModifiers = {
        'clay': { soilHealth: -5, carbonImpact: 5, waterEfficiency: -10, biodiversity: 0 },
        'loam': { soilHealth: 10, carbonImpact: 5, waterEfficiency: 10, biodiversity: 10 },
        'sandy': { soilHealth: -10, carbonImpact: -5, waterEfficiency: -15, biodiversity: -5 },
        'silt': { soilHealth: 0, carbonImpact: 0, waterEfficiency: 5, biodiversity: 0 }
    };

    // pH level optimization (optimal range 6.0-7.5)
    const phOptimal = 6.75;
    const phDifference = Math.abs(phLevel - phOptimal);
    const phPenalty = Math.min(phDifference * 10, 30); // Max 30% penalty

    // Get base scores
    const baseScores = cropScores[cropType] || cropScores['wheat'];
    const soilMods = soilModifiers[soilType] || soilModifiers['loam'];

    // Calculate final scores
    const metrics = {
        soilHealth: Math.max(0, Math.min(100, baseScores.soilHealth + soilMods.soilHealth - phPenalty)),
        carbonImpact: Math.max(0, Math.min(100, baseScores.carbonImpact + soilMods.carbonImpact - phPenalty * 0.5)),
        waterEfficiency: Math.max(0, Math.min(100, baseScores.waterEfficiency + soilMods.waterEfficiency - phPenalty * 0.7)),
        biodiversity: Math.max(0, Math.min(100, baseScores.biodiversity + soilMods.biodiversity - phPenalty * 0.3))
    };

    // Calculate overall sustainability score
    const overallScore = Math.round((metrics.soilHealth + metrics.carbonImpact + metrics.waterEfficiency + metrics.biodiversity) / 4);

    return {
        ...metrics,
        overallScore,
        recommendations: generateRecommendations(cropType, soilType, phLevel, metrics)
    };
}

// Generate recommendations based on the metrics
function generateRecommendations(cropType, soilType, phLevel, metrics) {
    ...
    if (phLevel < 6.0) {
        recommendations.push({
            type: 'pH',
            message: 'Consider adding lime to increase soil pH for better nutrient availability.',
            priority: 'high'
        });
    } else if (phLevel > 7.5) {
        recommendations.push({
            type: 'pH',
            message: 'Consider adding sulfur or organic matter to lower soil pH.',
            priority: 'medium'
        });
    }

    // Soil health recommendations
    if (metrics.soilHealth < 60) {
        recommendations.push({
            type: 'soilHealth',
            message: 'Implement cover cropping and reduce tillage to improve soil structure.',
            priority: 'high'
        });
    }

    // Water efficiency recommendations
    if (metrics.waterEfficiency < 50) {
        recommendations.push({
            type: 'water',
            message: 'Consider drip irrigation or mulching to improve water retention.',
            priority: 'high'
        });
    }

    // Biodiversity recommendations
    if (metrics.biodiversity < 50) {
        recommendations.push({
            type: 'biodiversity',
            message: 'Plant diverse cover crops and maintain field margins to support beneficial insects.',
            priority: 'medium'
        });
    }

    // Carbon impact recommendations
    if (metrics.carbonImpact < 60) {
        recommendations.push({
            type: 'carbon',
            message: 'Adopt no-till practices and integrate more perennial crops.',
            priority: 'medium'
        });
    }
...
function updateSustainabilityDisplay(metrics) {
    ...
    if (scoreValue && scoreLabel && scoreProgress) {
        ...
        if (metrics.overallScore >= 80) {
            ...
            label = 'Fair';
            color = '#f39c12';
        } else if (metrics.overallScore >= 20) {
            label = 'Poor';
            color = '#e67e22';
        }
    ...
    const metricNames = ['soilHealth', 'carbonImpact', 'waterEfficiency', 'biodiversity'];
    const metricLabels = ['Soil Health', 'Carbon Impact', 'Water Efficiency', 'Biodiversity'];
    ...
    metricNames.forEach((metricName, index) => {
        ...
        if (metricElements[index]) {
            ...
            const fillElement = metricElements[index].querySelector('.metric-fill');
            
            if (valueElement && fillElement) {
                const value = Math.round(metrics[metricName]);
                valueElement.textContent = `${value}%`;
                fillElement.style.width = `${value}%`;
                
                // Color based on score
                let fillColor = '#e74c3c';
                if (value >= 80) fillColor = '#2ecc71';
                else if (value >= 60) fillColor = '#27ae60';
                else if (value >= 40) fillColor = '#f39c12';
                else if (value >= 20) fillColor = '#e67e22';
    ...
    if (metrics.recommendations && metrics.recommendations.length > 0) {
        showRecommendations(metrics.recommendations);
    }
}
...
function showRecommendations(recommendations) {
    ...
    if (highPriorityRecs.length > 0) {
        const message = `High Priority: ${highPriorityRecs[0].message}`;
        showToast(message, 'warning');
    }
    
    // Could also show all recommendations in a modal or dedicated section
...
function updateAQIProjection(chart) {
    ...
    // Simulate improved AQI values based on sustainability practices
    const currentAQI = 280;
    const projectedData = [
        currentAQI,
        Math.max(50, currentAQI - 60),  // 3 months
        Math.max(40, currentAQI - 120), // 6 months
        Math.max(35, currentAQI - 180), // 9 months
        Math.max(30, currentAQI - 200)  // 12 months
    ];
