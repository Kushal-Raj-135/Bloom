...
const AI_API_URL = '/api/ai/recommendations';
...
const MapModule = {
    ...
    async handleSearch(query) {
        ...
        try {
            ...
            suggestions.style.display = 'block';            const response = await fetch(
                `/api/geocoding/search/${encodeURIComponent(query)}?limit=5`
            );
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function updateSelectedLocation(lat, lng) {
        try {
            const response = await fetch(`/api/geocoding/reverse/${lat}/${lng}`);
            const data = await response.json();
    ...
    mapSearchInput.addEventListener('input', function() {
        ...
        window.searchDebounceTimer = setTimeout(async () => {
            try {                mapSearchSuggestions.innerHTML = '<div class="loading">Searching...</div>';
                mapSearchSuggestions.style.display = 'block';
                
                const response = await fetch(`/api/geocoding/search/${encodeURIComponent(query)}?limit=5`);
                const data = await response.json();
    ...
    });    // Function to get AI recommendations from backend proxy
