...
const AQI_API_URL = '/api/weather/aqi';
const WEATHER_API_URL = '/api/weather/openweather';
...
async function getLocationName(lat, lon) {
    try {
        const response = await fetch(`/api/geocoding/reverse/${lat}/${lon}`);
        if (!response.ok) {
        ...
        return addressParts[0] + ', ' + addressParts[addressParts.length - 1].trim();
    } catch (error) {
...
async function getAQIData(lat, lon) {
    try {
        const response = await fetch(`${AQI_API_URL}/${lat}/${lon}`);
        const data = await response.json();
          if (data.status === 'ok') {
            return data.data;
        } else {
            console.warn('AQI API returned error status. Using mock data.');
...
async function getWeatherData(lat, lon) {
    try {        const response = await fetch(`${WEATHER_API_URL}/${lat}/${lon}?units=metric`);
        if (!response.ok) {
    ...
    try {
        ...
        if (navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(async (position) => {
                ...
                const response = await fetch(`${AQI_API_URL}/${latitude}/${longitude}`);
                const data = await response.json();
                if (data.status === 'ok') {
