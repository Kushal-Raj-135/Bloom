...
function formatRecommendations(data) {
    ...
    const sections = wrappedContent.split(/<h[1-6]>/);
    
    sections.forEach((section, index) => {
        if (section.trim()) {
            // Extract header text and content
            const headerMatch = section.match(/^(.*?)<\/h[1-6]>(.*)/s);
            
            if (headerMatch) {
                const headerText = headerMatch[1].trim();
                const content = headerMatch[2].trim();
                
                // Categorize sections and add appropriate styling
                if (headerText.toLowerCase().includes('year')) {
                    formattedHtml += `
                    ...
                    `;
                } else if (headerText.toLowerCase().includes('fertilizer') || headerText.toLowerCase().includes('organic')) {
                    formattedHtml += `
                    ...
                    `;
                } else if (headerText.toLowerCase().includes('additional') || headerText.toLowerCase().includes('recommendation')) {
                    formattedHtml += `
                ...
                // Content without header (first section)
                formattedHtml += `
