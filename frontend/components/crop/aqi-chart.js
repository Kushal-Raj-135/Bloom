...
function initializeAQIChart() {
  ...
  const gradientWithRecommendations = ctx
    .getContext("2d")
  ...
  const gradientWithoutRecommendations = ctx
    .getContext("2d")
    .createLinearGradient(0, 0, 0, 400);
...
function updateChartData(formData) {
  ...
  const followingRecommendations =
    formData.get("followRecommendations") === "yes";
  ...
  aqiChart.data.datasets[0].data = followingRecommendations
    ? newData
    : generateSampleData(true);

  // Update the second dataset (Without Recommendations)
  aqiChart.data.datasets[1].data = !followingRecommendations
    ? newData
