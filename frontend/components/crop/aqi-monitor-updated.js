// filepath: c:\Users\jai swrup\Desktop\code\Bloom\frontend\components\crop\aqi-monitor.js
// DOM Elements
  ...
  good: [
    {
      ...
      description:
        "Excellent conditions for planting seedlings and transplanting.",
      icon: "seedling",
    },
    {
      title: "Harvesting",
  ...
  moderate: [
    {
      ...
      description:
        "Focus on less strenuous activities and take regular breaks.",
      icon: "clock",
    },
    {
      title: "Irrigation",
      description:
        "Good time for watering crops and checking irrigation systems.",
      icon: "faucet",
    },
    {
      title: "Maintenance",
...
async function getLocationName(lat, lon) {
  try {
    ...
    if (!window.api || !window.api.getLocationName) {
...
async function getAQIData(lat, lon) {
  try {
    // Check if API is available
    if (!window.api || !window.api.getAQIByCoords) {
      console.error("API not loaded or getAQIByCoords function not available");
...
function updateAQI(aqiValue) {
  ...
  else if (aqiValue <= aqiData.unhealthySensitive.range[1])
...
function updatePollutantColors(iaqi) {
  const pollutants = {
    pm25: {
      element: pm25ValueEl,
      threshold: { good: 12, moderate: 35.4, unhealthy: 55.4 },
    },
    pm10: {
      element: pm10ValueEl,
    ...
    o3: {
      element: o3ValueEl,
      threshold: { good: 50, moderate: 100, unhealthy: 150 },
    },
    no2: {
      element: no2ValueEl,
...
function getWeatherWarnings(weather) {
  ...
  if (weather.main.temp > 35) {
    warnings.push({
      message:
        "High temperature alert: Take frequent breaks and stay hydrated.",
      severity: "high",
...
function updateHealthMessage(aqi) {
  const messages = {
    ...
    moderate:
      "Sensitive individuals should take breaks during extended outdoor work.",
    unhealthy:
      "Wear appropriate protection for outdoor activities and limit exposure.",
...
setInterval(async () => {
  try {
    if (window.api && window.api.getAQIByCoords) {
      const location = await getUserLocation();
      const data = await window.api.getAQIByCoords(location.lat, location.lon);
      if (data && data.aqi) {
        updateAQI(data.aqi);
      }
    } else {
