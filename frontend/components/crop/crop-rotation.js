...
const cropData = {
  ...
  tea: {
    ...
    benefits: {
      ...
      maize:
        "Maize has different nutrient requirements and helps break pest cycles.",
      potato:
        "Potatoes utilize different soil layers and break disease cycles.",
    },
    organicFertilizers: [
      {
        name: "Vermicompost",
      ...
      {
        name: "Neem Cake",
        description: "Natural pest deterrent and soil enricher.",
      },
      {
        name: "Bone Meal",
        description: "Provides phosphorus for root development.",
      },
    ],
  },
  sugarcane: {
...
const airQualityIndexGuidelines = {
  good: {
    status: "Good",
    color: "#009966",
    activities: [
...
function updateAQI(value) {
  ...
  const color =
    getComputedStyle(gaugeRing).getPropertyValue("--highlight-color");
...
async function handleFormSubmit(event) {
  ...
  try {
    ...
    const response = await fetch("/api/crops/recommendations", {
      method: "POST",
      headers: {
  ...
  sections.forEach((section) => {
    if (section.trim()) {
      ...
      if (section.includes("Year")) {
        ...
        if (cropLine) {
          cropLine = cropLine
            .replace("[Recommended crop name]", "")
            .replace("Recommended crop:", "")
            .trim();
          if (cropLine) {
            doc.setFontSize(13);
            doc.setTextColor(60, 60, 60);
